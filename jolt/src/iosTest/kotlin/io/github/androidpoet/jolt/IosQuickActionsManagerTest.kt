...
import io.github.androidpoet.jolt.internal.ShortcutItemsStore
import io.github.androidpoet.jolt.internal.ShortcutRecord
import io.github.androidpoet.jolt.internal.toItem
import io.github.androidpoet.jolt.internal.toRecord
...
class IosQuickActionsManagerTest {
    ...
    fun handleDecodesPublishedItemAndSynthesisesStaticOne() =
        runTest {
            ...
            assertEquals(QuickActionCodec.encode(a), published.userInfo?.get(JOLT_USER_INFO_KEY))
            val launch = Jolt.handle(published, createdScreen = true)
            assertEquals(a, launch.action)
            assertTrue(launch.createdScreen)
            assertTrue(Jolt.isDeliveryInstalled)
            ...
            val synthesised = Jolt.handle(static, createdScreen = false).action
            assertEquals(QuickAction("static", "Static", subtitle = "S", data = mapOf("k" to "v")), synthesised)

            val received = Jolt.manager.launches.first()
